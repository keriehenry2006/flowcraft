-- ==========================================
-- CZYSTA INSTALACJA: Process Executions (Bezpieczna)
-- Data: 2025-01-13
-- Opis: Usuwa wszystko i instaluje od nowa
-- ==========================================

-- 1. CLEANUP - Usuń wszystko co może powodować konflikty
DROP POLICY IF EXISTS "process_executions_access_policy" ON public.process_executions;
DROP POLICY IF EXISTS "process_executions_owner_policy" ON public.process_executions;
...
CREATE TABLE public.process_executions (
    ...
    CONSTRAINT fk_process_executions_process_id FOREIGN KEY (process_id) REFERENCES public.processes(id) ON DELETE CASCADE
);
...
-- 4. RLS - BARDZO PROSTE (tylko dla właścicieli projektów)
ALTER TABLE public.process_executions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "simple_process_executions_policy" ON public.process_executions
    FOR ALL
    USING (
        EXISTS (
...
-- 6. Funkcja do pobierania wykonań dla miesiąca
...
-- 7. Funkcja do aktualizacji statusu
...
-- 8. Migracja istniejących danych
INSERT INTO public.process_executions (
    process_id, 
    year, 
    month, 
...
-- 10. Podsumowanie
SELECT 
    'SUCCESS: Process Executions system installed successfully!' as status,
