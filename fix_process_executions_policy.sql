-- ==========================================
-- NAPRAWA: Usunięcie błędnej polityki RLS
-- Data: 2025-01-13
-- Opis: Naprawa błędu z pm.status w polityce RLS
-- ==========================================

-- 1. Sprawdź czy tabela już istnieje
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'process_executions') THEN
        RAISE NOTICE 'Tabela process_executions już istnieje - usuwam błędną politykę';
        
        -- Usuń błędną politykę jeśli istnieje
        DROP POLICY IF EXISTS "process_executions_access_policy" ON public.process_executions;
        
        -- Stwórz poprawną politykę
        CREATE POLICY "process_executions_owner_policy" ON public.process_executions
            USING (
                process_id IN (
                    SELECT p.id 
        ...
        RAISE NOTICE 'Polityka RLS naprawiona pomyślnie';
    ELSE
        RAISE NOTICE 'Tabela process_executions nie istnieje - kontynuuj z pełną migracją';
    END IF;
END $$;

-- 2. Jeśli tabela nie istnieje, stwórz ją teraz
...
-- 3. Indeksy (bezpiecznie)
CREATE INDEX IF NOT EXISTS idx_process_executions_process_id ON public.process_executions(process_id);
CREATE INDEX IF NOT EXISTS idx_process_executions_year_month ON public.process_executions(year, month);
CREATE INDEX IF NOT EXISTS idx_process_executions_status ON public.process_executions(status);
CREATE INDEX IF NOT EXISTS idx_process_executions_completed_by ON public.process_executions(completed_by);
CREATE INDEX IF NOT EXISTS idx_process_executions_deadline_date ON public.process_executions(deadline_date);

-- 4. RLS (bezpiecznie)
ALTER TABLE public.process_executions ENABLE ROW LEVEL SECURITY;

-- 5. Trigger funkcja (bezpiecznie)
...
-- 8. Migracja danych (bezpiecznie)
DO $$
BEGIN
    -- Sprawdź czy są jakieś procesy do migracji
    IF EXISTS (SELECT 1 FROM public.processes WHERE status != 'PENDING') THEN
        INSERT INTO public.process_executions (
            process_id, 
            year, 
            month, 
        ...
        RAISE NOTICE 'Migracja danych zakończona pomyślnie';
    ELSE
        RAISE NOTICE 'Brak danych do migracji';
    END IF;
END $$;
...
-- Wynik
SELECT 'Process Executions - migracja naprawiona i zakończona pomyślnie!' as result;