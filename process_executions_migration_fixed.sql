-- ==========================================
-- MIGRACJA: Dodanie tabeli process_executions (POPRAWIONA WERSJA)
-- Data: 2025-01-13
...
-- 3. Polityki RLS (Row Level Security) - Uproszczona wersja
ALTER TABLE public.process_executions ENABLE ROW LEVEL SECURITY;

-- Polityka dla właścicieli projektów (uproszczona - bez project_members)
...
CREATE TRIGGER IF NOT EXISTS trigger_process_executions_updated_at
    BEFORE UPDATE ON public.process_executions
    FOR EACH ROW
    EXECUTE FUNCTION update_process_executions_updated_at();

-- 5. Funkcja do pobierania wykonań procesów dla miesiąca
...
-- 6. Funkcja do aktualizacji statusu wykonania procesu
...
-- 7. Migracja istniejących danych z tabeli processes (bezpieczna)
DO $$
BEGIN
...
-- Koniec migracji - POPRAWIONA WERSJA
SELECT 'Migracja process_executions zakończona pomyślnie!' as result;